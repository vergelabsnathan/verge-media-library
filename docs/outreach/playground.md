...
```
https://playground.wordpress.net/?blueprint-url=https%3A%2F%2Fraw.githubusercontent.com%2Fvergelabsnathan%2Fvergelabs-media-library%2Fmain%2Fplayground%2Fblueprint.json
```
...
`playground/vergelabs-media-library.zip` is served from the repo instead. The community
...
```
git archive --format=zip --prefix=vergelabs-media-library/ -o dist/vergelabs-media-library-<v>.zip HEAD
cp dist/vergelabs-media-library-<v>.zip playground/vergelabs-media-library.zip
git add -f playground/vergelabs-media-library.zip
```
